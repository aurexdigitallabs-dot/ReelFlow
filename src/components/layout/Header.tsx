    ...
    <>
      <header className="sticky top-0 z-30 w-full bg-slate-950/90 backdrop-blur-md border-b border-slate-800/80 px-4 py-2.5 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <ReelFlowLogo size="sm" showSubtitle={true} />
          </button>

          {/* Store Selector Dropdown */}
          <div className="relative flex items-center">
            <Store className="w-4 h-4 text-indigo-400 absolute left-2.5 pointer-events-none" />
            <select
              ...
              className="pl-8 pr-7 py-1.5 text-xs font-semibold bg-slate-900 border border-slate-800 rounded-xl text-gray-200 focus:outline-none focus:border-indigo-500 appearance-none cursor-pointer hover:border-slate-700 transition-colors max-w-[160px] sm:max-w-none truncate"
            >
              <option value="all">All Accessible Brands ({visibleStores.length})</option>
              {visibleStores.map((s) => (
                <option key={s.id} value={s.id}>
                  {s.name} ({s.code})
                </option>
              ))}
              {/* Only Super Admin can add new stores! */}
              {canAddStore && <option value="__add_new__">+ Add New Brand/Store</option>}
            </select>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 bg-slate-900 border border-slate-800 hover:border-slate-700 rounded-xl text-xs font-semibold text-gray-200 transition-colors"
          >
            <User className="w-3.5 h-3.5 text-indigo-400" />
            <span className="hidden md:inline truncate max-w-[90px]">{userProfile?.displayName || userProfile?.email?.split('@')[0] || 'Sign In'}</span>
          ...
          <button
            type="button"
            onClick={toggleTheme}
            className="p-2 text-gray-400 hover:text-gray-200 hover:bg-slate-900 rounded-xl transition-colors"
          ...
          <button
            ...
            className="relative p-2 text-gray-300 hover:text-white hover:bg-slate-900 rounded-xl transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-semibold text-xs rounded-xl shadow-md shadow-indigo-600/30 transition-all hover:scale-[1.02] active:scale-[0.98]"
          >
